...
static void cmd_host_ram_info(struct vmm_chardev *cdev)
{
	...
	for (bn = 0; bn < bank_count; bn++) {
		...
		vmm_cprintf(cdev, "\n");
		vmm_cprintf(cdev, "Bank%02d Start      : %"PRIADDR"\n",
				bn, start);
		vmm_cprintf(cdev, "Bank%02d Size       : %"PRIADDR"\n",
				bn, size);
...
static void cmd_host_vapool_info(struct vmm_chardev *cdev)
{
	...
	vmm_cprintf(cdev, "Base Address : %"PRIADDR"\n", base);
...
static void cmd_host_vapool_bitmap(struct vmm_chardev *cdev, int colcnt)
{
	...
	for (ite = 0; ite < total; ite++) {
		if (umod32(ite, colcnt) == 0) {
			vmm_cprintf(cdev, "\n%"PRIADDR": ",
					(physical_addr_t)(base + ite * VMM_PAGE_SIZE));
		}
				    ...
				    int level, void *arg)
{
	...
	vmm_cprintf(cdev, "[0%016"PRIX64"-%016"PRIX64"] (0x%08x) %s\n",
