...
/*
 * When CONFIG_DEBUG_EMU is enabled, a field in the vmm_emudev structure
 * is added to hold debug flags.
 * The functions below are the only proper way to manipulate this field
 * properly, since it is absent when CONFIG_DEBUG_EMU is unset.
 *
 * Since those functions are inlined, branches that use the function
 * below can be optimized out, leading to zero overhead when CONFIG_DEBUG_EMU
 * is disabled.
 */
...
#else /* ! CONFIG_DEBUG_EMU */
static inline u32 vmm_devemu_get_debug_info(const struct vmm_emudev *edev)
{
	return 0;
}
#endif /* CONFIG_DEBUG_EMU */
